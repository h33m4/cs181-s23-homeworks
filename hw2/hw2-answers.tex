...
\subfloat[KNN Result]{\includegraphics[width = 3in]{hw2/knn_result.png}} 
\caption{Plots decision boundaries for classifiers a, b and c}
\label{fig:problem_3_1}
\end{figure}
\end{enumerate}

As we can see above, the decision boundaries for the 3 different models have different shapes. The linear softmax regression model has straight lines as boundaries, while model \textbf{b} has non-linear boundaries (given the $\ln$ and exponential functions in the basis, this makes sense.) The KNN boundary is a lot more 
